...
import com.developer.jw.ecosystemcopy.RetailEcoSystemApplication;
...
public class CircleMenuItemView extends RelativeLayout {
    ...
    public void onIconPressed(boolean bPressed) {
        ...
        if (mUpperTitle.isShown())
            currentTextView = mUpperTitle;
        else
            currentTextView = mLowerTitle;

        if (bPressed) {
            mIcon.setImageDrawable(mIconPressedDrawable);

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
                currentTextView.setTextColor(getResources().getColor(R.color.samsung_blue, RetailEcoSystemApplication.getContext().getTheme()));
            else
                currentTextView.setTextColor(getResources().getColor(R.color.samsung_blue));
        }
        else {
            mIcon.setImageDrawable(mIconDrawable);

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
                currentTextView.setTextColor(getResources().getColor(R.color.black, RetailEcoSystemApplication.getContext().getTheme()));
            else
                currentTextView.setTextColor(getResources().getColor(R.color.black));
        }
    }
}
