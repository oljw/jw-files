...
public class FileUtil {
        ...
        for (String ZipFileName : forUnzipList) {
            try {
                ...
                if (zipFile.exists()) {
                    ...
                    Log.e(TAG, "This device doesn't have a" + zipFile);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static Bitmap getBitmapResource(Context context, String filename) {
        String fullFilePath = getFileFullPath(context, filename);
        return BitmapFactory.decodeFile(fullFilePath);
    }

    public static String getTextFromResource(Context context, String filename) {
