...
public abstract class BaseActivity extends Activity implements ResourceUtil.MissingResourceListener {
    ...
    protected ArrayList<BaseFragment> mDeviceSpecFragments = new ArrayList<BaseFragment>();
    ...
    protected AppConsts.DeviceModel mDevice = null;
    protected AppConsts.DeviceModel mSelectedDevice = null;
    ...
    public void removeFragments(boolean deleteAll) {
        int remains = deleteAll ? 0 : 1;
        int start = mFragments.size() - 1 - remains;
        if (start <= 0) return;
    ...
    public void insertDeviceSpecFragment(BaseFragment fragment) {
        mDeviceSpecFragments.add(fragment);
    }
    public void removeDeviceSpecFragments(boolean deleteAll) {
        int remains = deleteAll ? 0 : 1;
        int start = mDeviceSpecFragments.size() - 1 - remains;

        for (int f = start; f>=0; f--) {
            FragmentTransaction ft = getFragmentManager().beginTransaction();
            ft.remove(mDeviceSpecFragments.get(f));
            ft.commitAllowingStateLoss();
            mDeviceSpecFragments.remove(f);
        }
    }
    ...
    DrawerLayout.DrawerListener drawerListener = new DrawerLayout.DrawerListener() {
        ...
        public void onDrawerOpened(View drawerView) {
            if (mDrawerStateListener != null)
                mDrawerStateListener.onDrawerOpened();
        }

        @Override
        public void onDrawerClosed(View drawerView) {
            if (mDrawerStateListener != null)
                mDrawerStateListener.onDrawerClosed();
        }
...
//        }
    ...
    DrawerStateListener mDrawerStateListener = null;
    public boolean closeDrawer(DrawerStateListener listener) {
...
//            }
    ...
    public void selectedDevice(String deviceModel) {
        mSelectedDevice = AppConsts.DeviceModel.valueOf(deviceModel);
    }

    public String getDeviceSpecJson(AppConsts.DeviceModel deviceModel) {
        switch (deviceModel) {
            case GALAXY_S4:
                return "models/galaxy_s4_spec.json";
            case GALAXY_S5:
                return "models/galaxy_s5_spec.json";
            case GALAXY_S6:
                return "models/galaxy_s6_spec.json";
            case GALAXY_S6_EDGE:
                return "models/galaxy_s6_edge_spec.json";
            case GALAXY_S6_EDGE_PLUS:
                return "models/galaxy_s6_edge_plus_spec.json";
            case GALAXY_S7:
                return "models/galaxy_s7_spec.json";
            case GALAXY_S7_EDGE:
                return "models/galaxy_s7_edge_spec.json";
            case GALAXY_NOTE3:
                return "models/galaxy_note3_spec.json";
            case GALAXY_NOTE4:
                return "models/galaxy_note4_spec.json";
            case GALAXY_NOTE5:
                return "models/galaxy_note5_spec.json";
            case GALAXY_NOTE_EDGE:
                return "models/galaxy_note_edge_spec.json";
        }
