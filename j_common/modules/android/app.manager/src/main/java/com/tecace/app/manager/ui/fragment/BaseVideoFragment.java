...
public abstract class BaseVideoFragment extends BaseFragment implements
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mVideoView != null) {
            ...
            // [[ chapter interactions including super
            if (getFragmentModel().getFragment().getChapterLayoutResId() != 0) {
                mVideoView.setChapterLayoutResId(getFragmentModel().getFragment().getChapterLayoutResId());
            }
            // ]]
            ...
            //add super text view
            ViewGroup videoContainer = (ViewGroup) mView.findViewById(R.id.video_container);
...
//    }
    ...
    @Override
    public void onVideoBreakPoint(VideoTextureView view, int nextChapter, final Chapter chapter) {
        ...
        if (m != null) {
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    try {
                        m.invoke(BaseVideoFragment.this, new Object[]{chapter});
                    } catch (Exception ex) {
    ...
    @Override
    public void onChapterEnded(VideoTextureView view, int chapterEndedIndex, final Chapter chapter) {
    ...
    protected void setForcedSeekToChapterEnd(int index) {
        Log.d(TAG, "next chapter is " + index);
        if (mVideoView != null) {
            if (mVideoView.hasChapters()) {
                if (index < mVideoView.getTotalChapters()) {
                    mVideoView.seekToChapterEnd(index);
                }
            }
        } else {
            Log.w(TAG, "VideoView is null");
